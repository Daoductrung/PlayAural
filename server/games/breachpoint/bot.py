...
ATTACK_STRATEGIES = (
    ...
    AttackStrategyProfile(ATTACK_STRATEGY_FAKE, 4),
)
...
class BreachPointBotCoordinator:
    ...
    def buy_action(self, game: BreachPointGame, bot: BreachPointPlayer) -> str | None:
        ...
        if not primary:
            ...
            if pistol_utility:
                return f"buy_utility_{pistol_utility.id}"
        ...
        if preferred_sidearm:
            return f"buy_weapon_{preferred_sidearm.id}"
    ...
    def _site_assignment_rotation(game: BreachPointGame, site_count: int) -> int:
        """Rotate defender seats across sites so initiative is not site-locked."""
    ...
    def _proactive_angle_action(
        ...
        attacking_site_control = bool(
            bot.team_index == SIDE_TERRORISTS
            and game.bomb_state == BOMB_CARRIED
            and execute_site
