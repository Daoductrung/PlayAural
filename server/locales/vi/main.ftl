...
admin-database-management-summary = Các công cụ bảo trì cơ sở dữ liệu. Những thao tác này tạm dừng ván chơi và các thay đổi tài khoản trên toàn máy chủ, đồng thời chỉ dành cho Nhà phát triển.
admin-database-backup = Sao lưu cơ sở dữ liệu
admin-database-backup-confirm = Sao lưu cơ sở dữ liệu ngay bây giờ? Ván chơi và các thay đổi tài khoản sẽ tạm dừng trong khi SQLite tạo và kiểm tra một bản phục hồi. Bản sao lưu sẽ được giữ trong thư mục sao lưu của máy chủ cho đến khi người vận hành xóa thủ công.
admin-database-backup-success = Đã sao lưu cơ sở dữ liệu: { $filename }, { $size } byte.
...
admin-database-compact-success = Đã thu gọn cơ sở dữ liệu. Kích thước tệp thay đổi từ { $before } byte xuống { $after } byte; đã thu hồi { $reclaimed } byte. Bản sao lưu an toàn: { $filename }.
